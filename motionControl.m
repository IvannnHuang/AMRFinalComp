function [dataStore] = motionControl(Robot, maxTime)
 ...
 dataStore.pose = [[-3.5, 1.5, deg2rad(0)]];
 count = 0;
 ...
 while toc < maxTime
     ...
     deltaD = dataStore.odometry(end, 2);  % distance moved
     deltaA = dataStore.odometry(end, 3);  % angle turned
     ...
     [pose] = integrateOdom(dataStore.pose(end, :), deltaD, deltaA);
